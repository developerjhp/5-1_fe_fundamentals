...
- [x] 빈 시간대 클릭 시 예약 생성 페이지(`/reservations/new`)로 이동 (회의실, 날짜, 시간 정보를 적절한 방식으로 전달)
  - 쿼리 스트링을 이용하는 방식을 사용
    - 새로고침 시에도 폼 초기값이 유지되는 장점
    - 추가 의존성은 없음 (zustand라던가)
    - 민감 정보가 아니므로 url 노출에 문제 없음
    - 단점은, url 수정 시 다른 데이터가 들어가는 데 이건 이 방식의 한계.
      - 어차피 서버에서 검증이 들어가기 때문에, 큰 문제는 없다 생각함.
...
- 클라이언트 검증 (zod + react-hook-form)
  - 빈 값, 시작 시간 >= 종료 시간, 과거 시간 등 기본 유효성은 클라이언트에서 처리

- 서버 검증 (409 응답 처리)
  - 실제 충돌 여부는 서버에 위임. 409 응답 시 충돌 정보(conflictWith)를 폼에 표시

- 낙관적 업데이트를 쓰지 않은 이유
  - 예약 생성은 서버 응답 후 페이지를 이동하는 흐름이라, 낙관적으로 미리 반영할 화면 자체가 없음
  - 충돌 가능성이 있는 액션에 낙관적 업데이트를 적용하면, 성공한 척 UI를 보여줬다가 409로 롤백하는 비용이 그냥 기다리는 것보다 오히려 큼
  - 근본적으로 낙관적 업데이트는 나 자신의 UI를 서버 응답 전에 미리 바꾸는 것일 뿐, 다른 사용자의 동시 예약으로 인한 충돌은 클라이언트가 알 수 없음. 이는 실제 서버뿐 아니라 localStorage 기반의 Mock API도 동일한데, localStorage는 탭 간에 공유되지만 각 탭의 TanStack Query 캐시는 독립적이므로, 다른 탭에서 발생한 변경을 실시간으로 감지하려면 결국 SSE나 WebSocket 같은 별도의 실시간 동기화 메커니즘이 필요한 별개의 문제라고 생각했음
  - 따라서 충돌 검증은 서버에 완전히 위임하고, 클라이언트는 409 응답을 받았을 때 사용자에게 충돌 정보를 안내하는 것으로 처리

### 2. 상태 관리 전략

> 예약 데이터를 어떻게 관리하는지? (TanStack Query, useState, 전역 store 등)

- tanstack query를 이용하여 관리. 서버 상태이므로 hook의 데이터를 진실의 원천으로 두고 관리함
  - 일단은 굳이 전역 상태로 따로 클라이언트 상태를 두고, 서버 상태와 동기화를 할 필요는 없다고 생각했음
    > 예약 생성/취소 후 여러 페이지의 데이터를 어떻게 동기화하는지?
- 예약 생성 / 취소 후 invalidateQueries를 이용하여 데이터 동기화를 하는 편.
- 만약 여러 탭을 열어두었고, 그것마저 동기화가 되어야 한다면, refetchOnFocus? 등의 옵션을 사용할듯.
  - 다만, 이 케이스의 경우, suspense query가 맞는지 잘 모르겠긴 함... 뭔가 잘 안되었던 것 같은 기억이 (캐싱 떄매)

### 3. 폼 상태 관리

> 예약 생성 폼의 상태를 어떻게 관리했는지?

- 폼은 react-hook-form을 사용. -> 쉽게 form을 관리할 수 있고, 유효성 검사 연동이 쉽고, form 성능 최적화까지 됨.
  > 유효성 검증은 어디서, 어떻게 처리했는지?
- 지금 고민중.. 단일 input 등에서 처리할수도 있고, react-hook-form과 zod 등을 이용해서 할 수도 있다고 봄.
...
- react-router-dom
  - 총 4개의 페이지가 있으니, router 이동을 하기 위함
- react-hook-form
  - 폼 상태 관리 전용 도구
- zod
  - 유효성 검사를 위한 도구
- tailwind css
  - styling을 위한 도구
- ky: fetch기반의 http기반 요청을 쉽게 하기 위한 도구.
